...
{   
	...
	// New scene to transition to when fade out complete
	static string NewScene = "";
	static float TransitionTime;

	public static void StartAlphaFade(Color newScreenOverlayColor, bool isFadeIn, float fadeDuration, string sceneToGoTo )
	{
		NewScene = sceneToGoTo;
		TransitionTime = Time.time + (fadeDuration / 2);
