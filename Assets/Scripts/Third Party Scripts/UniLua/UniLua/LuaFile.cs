...
namespace UniLua
{
	internal class LuaFile
	{
		...
		{
			var path = System.IO.Path.Combine(GetLuaRoot(), filename);
		...
		private static string GetLuaRoot()
		{
			var task = UnityThreadHelper.Dispatcher.Dispatch(() => { return Application.streamingAssetsPath; });
			var result = task.Wait<string>();
			return System.IO.Path.Combine(result, "LuaRoot");
		}
	}
